...
async function getContent(req, res, next) {
    try {
        ...
        const searchRegexExp = new RegExp(value.searchTxt, 'gi');

        const contactCount = await Contact.find({ name: searchRegexExp }).countDocuments();
        const contact = await Contact.find({ name: searchRegexExp }, '-createdAt -updateAt -createdBy -__v')
            .skip(querySkipNo)
            .limit(queryLimitNo);
